...
public class ForgotPasswordController {
    ...
    public String procesarEnvioToken(@RequestParam("email") String email, HttpServletRequest request,RedirectAttributes redirectAttributes) {
        try {
            ...
            String link = urlBase + "/reset_password?token=" + token;
    ...
    public String mostrarFormularioReset(@RequestParam("token") String token, Model model) {
        ...
        if (usuario == null) {
            ...
            return "usuario/forgot_password_form";
        }
    ...
    private void enviarEmail(String correoDestino, String link) throws MessagingException {
        ...
        String contenido = "<div style='font-family: Arial, sans-serif; border: 1px solid #ddd; padding: 20px;'>"
                         ...
                         + "<a href='" + link + "' style='background-color: #28a745; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;'>Cambiar Contraseña</a>"
        ...
        try {
            ...
            e.printStackTrace(); // imprime la causa real del error
        }
    }
}